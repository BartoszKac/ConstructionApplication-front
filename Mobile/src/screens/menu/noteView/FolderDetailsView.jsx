...
import { View, Text, StyleSheet, FlatList, Image, TouchableOpacity, SafeAreaView, ActivityIndicator, ScrollView, Modal, Linking, Alert } from 'react-native';
...
function FolderDetailsView({ route, navigation }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error("Błąd pobierania danych folderu:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteProduct = async (productIndex) => {
    Alert.alert("Usuwanie", "Czy na pewno chcesz usunąć ten produkt?", [
      ...
      { 
        ...
        onPress: async () => {
          try {
            ...
            if (idx !== -1) {
              allFolders[idx].savedResults.splice(productIndex, 1);
              await saveFolders(allFolders);
              setSavedProducts([...allFolders[idx].savedResults]);
  ...
  if (!folderId) {
    return (
      <SafeAreaView style={styles.container}>
        <Text style={styles.empty}>Błąd: Nie odnaleziono folderu.</Text>
      </SafeAreaView>
    );
  }
      ...
      {loading ? <ActivityIndicator size="large" style={{marginTop: 50}} color="#4CAF50"/> : (
        <ScrollView contentContainerStyle={{paddingBottom: 100}}>
          ...
          {savedProducts.map((item, index) => (
            <TouchableOpacity key={index} style={styles.prodCard} onPress={() => openDetails(item, index)}>
              <View style={{flex: 1}}>
            ...
            {selectedProduct && (
              <ScrollView style={styles.modalScroll}>
                ...
                <View style={styles.detailsBox}>
                  {selectedProduct.allDetails && Object.entries(selectedProduct.allDetails).map(([key, value]) => {
                    const skip = ["image_imageUrl", "itemWebUrl", "url", "itemId", "currency", "mainImage", "title"];
                ...
                <View style={{gap: 10, paddingBottom: 20}}>
                  {(selectedProduct.allDetails?.itemWebUrl || selectedProduct.allDetails?.url) && (
                    <TouchableOpacity 
                      style={styles.linkBtn} 
                      onPress={() => Linking.openURL(selectedProduct.allDetails.itemWebUrl || selectedProduct.allDetails.url)}
                    >
                  ...
                  <TouchableOpacity 
                    style={[styles.linkBtn, {backgroundColor: '#ff4757'}]} 
                    onPress={() => deleteProduct(selectedProduct.index)}
                  >
                    <Text style={styles.linkBtnText}>Usuń z projektu 🗑️</Text>
                  </TouchableOpacity>
                </View>
              </ScrollView>
            )}
...
const styles = StyleSheet.create({
  ...
  prodCard: { flexDirection: 'row', padding: 15, backgroundColor: '#fff', marginHorizontal: 10, marginTop: 10, borderRadius: 12, elevation: 3 },
  ...
  linkBtn: { backgroundColor: '#4CAF50', padding: 15, borderRadius: 12, alignItems: 'center' },
